...
export function getTranslations (parent) {
  return new Promise((resolve, reject) => {
    MongoClient.connect(url, (err, db) => {
      ...
      tweets.findOne({tweetId: parent}, (err, tweet) => {
        if (err) reject(err)
    ...
    MongoClient.connect(url, (err, db) => {
      ...
      tweets.find({}).toArray((err, tweets) => {
        ...
        for (const tweet of tweets) {
          translations.push(...tweet.translations)
        }
        ...
        if (translations.length > 5) {
          resolve(translations.slice(0, 4))
        } else {
...
export function getTranslationsByUserID (uid) {
  return new Promise((resolve, reject) => {
    MongoClient.connect(url, (err, db) => {
      ...
      tweets.find({'translations.uid': uid.toString()}).toArray((err, tweets) => {
        if (err) resolve(err)
        db.close()
